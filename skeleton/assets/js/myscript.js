...
data.then(function (data) {
	...
	data.forEach(function (d, i) {
		...
		var longitude = parseFloat(d.longitude) + Math.random(); //Counter the exact same position on map eg. Paris
		var latitude = parseFloat(d.latitude) + Math.random();
		...
		popupContent.name = 'frame-' + d.slug_game + d.event + d.medal_type;
		popupContent.src = "assets/html/popup.html";
		popupContent.width = w * 0.4;
		popupContent.height = h * 0.4;
		popupContent.id = d.slug_game + d.event + d.medal_type;
		...
		minimumYear = Math.min(minimumYear, parseInt(d.slug_game.split("-")[0]));
		maximumYear = Math.max(maximumYear, parseInt(d.slug_game.split("-")[0]));

		professionOptions.push(d.discipline);
	});
